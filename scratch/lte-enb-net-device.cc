...
LteEnbNetDevice::ReadControlFile()
{
    ...
    if (m_controlFilename.empty())
    {
        return;
    }

    // -------- Semáforos (inalterado) --------
    if (m_useSemaphores)
    {
    ...
    // ======= MODO HIERÁRQUICO: TS + ES no mesmo CSV =======
    if (m_controlFilename.find("hierarchical_actions.csv") != std::string::npos)
    {
        bool headerHandled = false;
        bool anyEsChange = false;
        long long maxTsMs = 0;
        ...
        while (std::getline(csv, line))
        {
            ...
            // Pula cabeçalho se houver (procuro por "action_type")
            if (!headerHandled)
            {
                headerHandled = true;
                if (line.find("action_type") != std::string::npos)
                {
                    // é cabeçalho — leia a próxima linha
                    continue;
                }
            }
            ...
            // timestamp (segundos com ponto, senão ms)
            std::getline(ss, cell, ',');
            long long ts_ms = 0;
            if (!cell.empty())
            {
                ...
                {
                    double ts_s = std::stod(cell);
                    ts_ms = static_cast<long long>(ts_s * 1000.0 + 0.5);
                }
                else
                {
                    ts_ms = std::stoll(cell);
                }
            }
            if (ts_ms > maxTsMs) { maxTsMs = ts_ms; }

            // action_type
            std::getline(ss, cell, ',');
            std::string actionType = cell;

            if (actionType == "TS_HANDOVER")
            {
                // imsi
                std::getline(ss, cell, ',');
                uint64_t imsi = std::stoull(cell);

                // target_cell_id
                std::getline(ss, cell, ',');
                uint16_t targetCellId = static_cast<uint16_t>(std::stoul(cell));

                NS_LOG_INFO("TS_HANDOVER ts=" << ts_ms << "ms imsi=" << imsi
                ...
                {
                    Simulator::Schedule(MilliSeconds(ts_ms),
            ...
            else if (actionType == "ES_SET_STATE")
            {
                // pular campos imsi e target_cell_id
                std::getline(ss, cell, ','); // imsi (vazio)
                std::getline(ss, cell, ','); // target_cell_id (vazio)

                // bs_id
                std::getline(ss, cell, ',');
                uint16_t cellId = static_cast<uint16_t>(std::stoul(cell));

                // es_state
                std::getline(ss, cell, ',');
                std::string esState = cell;

                // Convenção: ON => hoAllowed=1; IDLE/SLEEP/OFF => hoAllowed=0
                bool hoAllowed = (esState == "ON");

                NS_LOG_INFO("ES_SET_STATE ts=" << ts_ms << "ms cellId=" << cellId
                << " esState=" << esState
                << " hoAllowed=" << hoAllowed);
            ...
            {
                NS_LOG_WARN("Unknown/unsupported action_type in hierarchical file: "
                << actionType << " (line: " << line << ")");
            }
        }

        // Se houve mudanças ES, dispare a evacuação depois do último timestamp
        if (anyEsChange)
        {
            if (m_scheduleControlMessages)
            {
                Simulator::Schedule(MilliSeconds(maxTsMs + 1),
    ...
    { // TODO antigo: agora aplico o timestamp quando m_scheduleControlMessages==true
        long long timestamp{};
        while (std::getline(csv, line))
        {
            if (line == "") { continue; }
            ...
            uint64_t imsi;
            std::getline(lineStream, cell, ',');
            imsi = std::stoull(cell); // <- corrigido

            uint16_t targetCellId;
            std::getline(lineStream, cell, ',');
            targetCellId = static_cast<uint16_t>(std::stoul(cell));

            NS_LOG_INFO("Handover command for timestamp " << timestamp
            << " imsi " << imsi
            << " targetCellId " << targetCellId);
            ...
            {
                Simulator::ScheduleWithContext(m_node->GetId(), // <- corrigido
                                               Seconds(0),
                                               &LteEnbRrc::PerformHandoverToTargetCell,
                                               m_rrc, imsi, targetCellId);
            }
    ...
    // ======= MODO LEGADO: ES =======
    if (m_controlFilename.find("es_actions_for_ns3.csv") != std::string::npos)
    {
        long long timestamp{};
        uint8_t counter = 0;
        while (std::getline(csv, line))
        {
            ...
            uint16_t cellId;
            std::getline(lineStream, cell, ',');
            cellId = static_cast<uint16_t>(std::stoul(cell));

            bool hoAllowed;
            std::getline(lineStream, cell, ',');
            hoAllowed = static_cast<bool>(std::stoi(cell));
        ...
        if (counter > 0)
        {
            ...
            {
                Simulator::Schedule(MilliSeconds(timestamp + 1),
                                    &LteEnbRrc::EvictUsersFromSecondaryCell, m_rrc);
            }
        }
    ...
    // ======= MODO LEGADO: QoS =======
    if (m_controlFilename.find("qos_actions.csv") != std::string::npos)
    {
        long long timestamp{};
        ...
        while (std::getline(csv, line))
        {
            ...
            uint16_t ueId;
            std::getline(lineStream, data, ',');
            ueId = static_cast<uint16_t>(std::stoul(data));
        ...
        {
            uint16_t ueId = uePercentage.first; // já é a chave do CSV legado
    ...
    // -------- Desconhecido --------
    csv.close();
    NS_FATAL_ERROR("Unknown use case not implemented yet with filename: " << m_controlFilename);
}
